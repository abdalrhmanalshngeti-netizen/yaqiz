  ...
  try {
    ...
    if (linked_invoice_id) {
      const { rows: [inv] } = await db.query(
        `SELECT id FROM invoices WHERE id = $1 AND company_id = $2`, [linked_invoice_id, req.user.company_id]
      );
      if (!inv) return res.status(404).json({ success: false, message: 'الفاتورة المرتبطة غير موجودة' });
    }
    if (linked_purchase_id) {
      const { rows: [pur] } = await db.query(
        `SELECT id FROM purchases WHERE id = $1 AND company_id = $2`, [linked_purchase_id, req.user.company_id]
      );
      if (!pur) return res.status(404).json({ success: false, message: 'المشتريات المرتبطة غير موجودة' });
    }
    ...
    const { rows: [ret] } = await db.query(`
      ...
      RETURNING *
    `, [req.user.company_id, return_no, type, party_name || '', product_id || null,
